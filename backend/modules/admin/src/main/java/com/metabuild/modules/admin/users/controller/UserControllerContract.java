...
public final class UserControllerContract {
 ...
 public static final class UpdateUserWrite {private boolean np,dp,rp,pp,ep,sp;private String name,role,phone,email,status;private UUID deptId;public void setName(String v){np=true;name=v;}public void setDeptId(UUID v){dp=true;deptId=v;}public void setRole(String v){rp=true;role=v;}public void setPhone(String v){pp=true;phone=v;}public void setEmail(String v){ep=true;email=v;}public void setStatus(String v){sp=true;status=v;}public UserPatch toPatch(){if((np&&(name==null||name.isBlank()))||(dp&&deptId==null)||(rp&&(role==null||role.isBlank()))||(ep&&(email==null||!email.contains("@")))||(sp&&(status==null||!Set.of("active","disabled","unactivated","left").contains(status))))throw invalid();return new UserPatch(np,name,dp,deptId,rp,role,pp,phone,ep,email,sp,status);}}
