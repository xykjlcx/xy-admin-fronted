...
public final class JdbcRefreshTokenStore implements RefreshTokenStore {
    ...
    @Override public String issue(UUID userId) { return issue(userId, ids.generate(),currentRevision(userId)); }
    @Override public String issue(UUID userId,long credentialRevision){return issue(userId,ids.generate(),credentialRevision);}
    ...
    private String issue(UUID userId, UUID familyId,long credentialRevision) {
        ...
        jdbc.update("insert into mb_refresh_token(id,user_id,family_id,token_hash,expires_at,credential_revision) values (?,?,?,?,?,?)",
                ids.generate(), userId, familyId, hash(raw), OffsetDateTime.ofInstant(clock.instant().plus(lifetime), ZoneOffset.UTC),credentialRevision);
        return raw;
    }
    ...
    @Override public RefreshRotationOutcome rotate(String token) {
        return transactions.execute(status -> {
            ...
            var rows = jdbc.query("""
                    select t.id,t.user_id,t.family_id,t.expires_at,t.consumed_at,t.revoked_at,t.credential_revision,
                           u.status as user_status,u.deleted_at as user_deleted_at,u.credential_revision as user_credential_revision
                    ...
                    "ACTIVE".equals(rs.getString("user_status")) && rs.getObject("user_deleted_at") == null,
                    rs.getLong("credential_revision"),rs.getLong("user_credential_revision")), hash(token));
            ...
            if(old.credentialRevision!=old.currentCredentialRevision){jdbc.update("update mb_refresh_token set revoked_at=current_timestamp where family_id=? and revoked_at is null",old.familyId);return RefreshRotationOutcome.rejected();}
            jdbc.update("update mb_refresh_token set consumed_at=current_timestamp where id=?", old.id);
            String replacement = issue(old.userId, old.familyId,old.credentialRevision);
            ...
            return RefreshRotationOutcome.success(new RefreshRotation(old.userId, replacement,old.credentialRevision));
        });
    }
    ...
    @Override public void revokeFamily(String token){jdbc.update("update mb_refresh_token set revoked_at=current_timestamp where family_id=(select family_id from mb_refresh_token where token_hash=?) and revoked_at is null",hash(token));}
    private long currentRevision(UUID userId){Long value=jdbc.queryForObject("select credential_revision from mb_user where id=? and deleted_at is null",Long.class,userId);if(value==null)throw new IllegalArgumentException("user not found");return value;}
    ...
    private record TokenRow(UUID id, UUID userId, UUID familyId, OffsetDateTime expires, OffsetDateTime consumed, OffsetDateTime revoked, boolean userEnabled,long credentialRevision,long currentCredentialRevision) {}
}
