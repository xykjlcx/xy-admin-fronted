...
public final class JdbcUserRepository implements UserRepository {
    ...
    public UserView update(UUID id,UserPatch p){if(p.empty())throw new BadRequest(()->"request.validation.failed","User patch is empty");UUID roleId=p.rolePresent()?resolveActiveRole(p.role()):null;List<String> sets=new ArrayList<>();List<Object> args=new ArrayList<>();if(p.namePresent()){sets.add("display_name=?");args.add(p.name());}if(p.deptPresent()){sets.add("dept_id=?");args.add(p.deptId());}if(p.phonePresent()){sets.add("phone=?");args.add(p.phone());}if(p.emailPresent()){sets.add("email=?");args.add(p.email());}sets.add("updated_at=current_timestamp");args.add(id);int n=jdbc.update("update mb_user set "+String.join(",",sets)+" where id=? and deleted_at is null",args.toArray());if(n==0)throw new NotFound(()->"iam.user.not-found","User not found");if(p.rolePresent()){jdbc.update("delete from mb_user_role where user_id=?",id);jdbc.update("insert into mb_user_role(user_id,role_id) values(?,?)",id,roleId);}return find(id).orElseThrow();}
    public Set<UUID> softDelete(UUID id){return changed("update mb_user set deleted_at=current_timestamp,status='DISABLED' where id=? and deleted_at is null returning id",id);}
    public Set<UUID> disable(Set<UUID> ids){checkBatch(ids);return changed("update mb_user set status='DISABLED' where id=any(?) and deleted_at is null and status<>'DISABLED' returning id",(Object)ids.toArray(UUID[]::new));}
    public Set<UUID> enable(Set<UUID> ids){checkBatch(ids);return changed("update mb_user set status='ACTIVE' where id=any(?) and deleted_at is null and status<>'ACTIVE' returning id",(Object)ids.toArray(UUID[]::new));}
    public Set<UUID> moveToDepartment(Set<UUID> ids,UUID dept){checkBatch(ids);return changed("update mb_user set dept_id=? where id=any(?) and deleted_at is null returning id",dept,(Object)ids.toArray(UUID[]::new));}
