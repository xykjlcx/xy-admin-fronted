...
class IamManagementContractTest {
 @Test void writeControllersAreProductionBeansAfterRuntimeProtocolIsComplete(){for(Class<?> type:List.of(UserControllerContract.class,DepartmentControllerContract.class,RoleControllerContract.class))assertThat(type.isAnnotationPresent(RestController.class)).isTrue();}
 ...
 @Test void userCreationInitializesExactlyThePreallocatedUser(){UUID id=UUID.randomUUID(),dept=UUID.randomUUID();FakeUsers repo=new FakeUsers(id);RecordingRefresh refresh=new RecordingRefresh();UserService service=new UserService(repo,refresh);service.create(id,"Alice",dept,"SYSTEM_ADMIN","1","a@example.test");assertThat(refresh.users).containsExactly(id);assertThat(refresh.initialized).isTrue();}
 ...
 static final class RecordingRefresh implements AuthorizationRefreshService {Set<UUID> users=Set.of();Cause cause;boolean initialized;public <T>T execute(Cause c,AuthorizationChange<T> change){cause=c;users=Set.copyOf(change.affectedUserIds());return change.mutate();}public <T>T executeInitialize(AuthorizationChange<T> change){initialized=true;return AuthorizationRefreshService.super.executeInitialize(change);}public <T>T executeTerminal(TerminalChange<T> change){cause=Cause.USER_CHANGED;users=Set.copyOf(change.affectedUserIds());return change.mutate();}}
