...
- Desktop：按 `docs/desktop.md` 替换 `desktop.config.ts` 的 appId/产品名/版权/Team ID/publisher 与 `build/icon.png`。占位身份可构建本地开发包，不能用于正式发布。
...
需要 Desktop 时，注入生产 API/Web public/update 三个 HTTPS URL，分别执行 `native` 和 `integrated` 的 `build:desktop`；选定最终窗口模式后，只发布该模式对应的签名安装包和 feed。
