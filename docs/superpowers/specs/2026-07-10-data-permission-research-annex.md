...
4. **PG RLS 不做主力**:表 owner 默认绕过(不加 FORCE 形同虚设)、逐行求值性能退化、SQL 表达不了外部上下文、难测难 CI。正确定位是防御纵深兜底,应用层过滤是主流。
5. **升级路径**(同一 jOOQ Listener 拦截点内):v1 角色级 + fail-closed → v2 资源默认策略 → v3 共享规则(例外放开)→ v4 策略引擎(OPA partial evaluation)。触发信号:同一角色 ≥2 资源确需不同 scope / 跨部门条件共享 / 非部门维度隔离 / 外部上下文参与判定。
