...
| 租户 | **单租户 B 端,不留 tenant_id 字段**(已确认),升级路径写 ADR:真 SaaS 需求出现时加列+回填+路由一次做全 | 前端契约无租户概念;"预留字段无路由"是假安全 |
...
Spec 引擎 / AI 循环引擎;多租户(含 tenant_id 占位字段);资源级数据权限与共享规则(路径已预留);SSO/LDAP/OIDC、2FA;消息队列、微服务;SSE/WebSocket(消息中心 v1 轮询);可配置项目生成器;SBOM/漏洞门禁(pre-prod)。
