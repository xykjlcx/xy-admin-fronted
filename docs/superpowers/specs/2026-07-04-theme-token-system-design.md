...
- 日期：2026-07-04
- 状态：v2 修订版（第四轮代码实况对抗评审已回写；值表 ⚠ 提案值待浏览器视觉定稿后冻结）
...
### 10.2 Token 值表（差异矩阵的可执行形态）

规则：默认列 = `:root` 忠实现状（即 feishu 观感）；覆盖列只写换系差异，`—` 表示无覆盖（吃来源 1/2 的自动差异）；
⚠ 为提案值，**切片实施时必须浏览器视觉确认后才能定稿**，不得凭表直接 commit。
dark 列缺省为 `—`（primitive dark block 自动生效），仅列出确需 dark 覆盖的行。

**Field 族**：

| token | 默认（现状忠实） | claude 覆盖 | shadcn 覆盖 |
| --- | --- | --- | --- |
| --field-bg | var(--surface) | ⚠ light: `#faf8f2`（暖纸底，与 focus 白底形成层次） | — |
| --field-bg-focus | var(--surface) | ⚠ light: `#ffffff` | — |
| --field-shadow | var(--shadow-card-sm) | ⚠ `none`（阴影更轻的体系） | ⚠ `none`（官方无控件阴影） |
| 其余 13 个 | §9.2 默认值 | — | — |
...
| --- | --- | --- | --- |
| --option-bg-highlighted | var(--pri-soft) | — | ⚠ var(--surface-2)（中性系） |
| --option-fg-highlighted | var(--pri) | — | ⚠ var(--text) |
| --option-bg-selected | var(--pri-soft) | — | ⚠ var(--surface-2) |
| --option-fg-selected | var(--pri) | — | ⚠ var(--text)（靠 check 图标区分选中） |
| 其余 | §9.3 默认值 | — | — |
...
| token | 默认 | claude 覆盖 | shadcn 覆盖 |
| --- | --- | --- | --- |
| --button-secondary-bg-hover | var(--surface) | — | ⚠ var(--surface-2)（官方 outline hover = accent 底） |
| --button-secondary-fg-hover | var(--pri) | — | ⚠ var(--text) |
| --button-secondary-border-hover | var(--pri) | — | ⚠ var(--control-border)（hover 不描主色边） |
| 其余 24 个 | §9.4 默认值 | — | — |

**其余族**（Tabs/Overlay/Table/Choice/Skeleton）：默认值即 §9 各节，暂无已确认的换系差异；
各切片实施时按本表格式补值表，若整族无覆盖则记"无覆盖"一行——这本身就是有效结论。

### 10.3 值表纪律

1. 值表是差异矩阵的唯一形态，禁止再写"暖白 / 低对比 / 接近官方"这类不可执行的形容词描述。
2. ⚠ 提案值的视觉定稿由人确认（Agent Browser 截图对比三 flavor），不得由模型凭描述生成后直接落地。
...
1. 三 flavor 完整基础状态 token，§10.2 值表全部定稿（无遗留 ⚠）。
...
```text
Step 0  冻结 + 在途改动盘点入基线
Step 1  机制与守护（accent runtime、两个 guard、/dev/theme-states 骨架）—— 不落组件 token
Step 2  Field Family（第一个四位一体垂直闭环，状态机钩子在此落地）
Step 3  Button Family（icon token 供 Step 4 引用，跨族依赖显式化）
Step 4  Overlay Family（Dialog 嵌套合同 + close 统一）
Step 5  Option / Menu Family（--accent 语义裁决锁定在此）
Step 6  Tabs（seg/line 分维）/ Choice / Skeleton-Empty
Step 7  Table / Pro / Shell / 样板页面收敛（页面层实际余量小）
Step 8  文档 + guard 转正 + 状态页补齐
```

时间有限时的第一阶段最小闭环：**Step 1 + Step 2 + Step 3 + Step 4** + 菜单管理弹窗浏览器验证
（相比 v1 的"1+2+4"增加 Step 3，因 Overlay close token 依赖 Button icon token，跳过会造成悬空引用）。
