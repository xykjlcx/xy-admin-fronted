...
- 日期：2026-07-04
- 状态：v4（第七轮三体系深挖调研 + 四项拍板已回写 §10.2b/§16.7；原始数据存档 docs/design/research/；Step 0-2R 已完成验收，Step 3-8 Codex 实施中（按 v3.1 合同），Step 9 精修切片待 Step 3-8 验收后开工）
...
Step 3   Button Family（含 v3 实测值 + 字重 token；icon token 供 Step 4）   ← Codex 实施中（按 v3.1 合同）
Step 4   Overlay Family（Dialog 嵌套合同 + close 统一）                     ← Codex 实施中
Step 5   Option / Menu Family（--accent 语义裁决；实施前补采三站点选项态）    ← Codex 实施中
Step 6   Tabs（seg/line 分维）/ Choice / Skeleton-Empty（实施前补采）        ← Codex 实施中
Step 7   Table / Pro / Shell / 样板页面收敛（含表格密度拍板结果）             ← Codex 实施中
Step 8   文档 + guard 转正 + 状态页补齐                                    ← Codex 实施中
Step 9   精修切片：值表 v4（§10.2b）统一落地                                ← Step 3-8 验收后开工
         任务清单：
