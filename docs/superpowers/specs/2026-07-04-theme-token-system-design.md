...
- 日期：2026-07-04
- 状态：v4.1（Step 3-8 已验收（有条件通过，2026-07-05），缺陷全部转入 Step 9；三项验收拍板已回写：三族状态机补钩子（§17-Step9-7）、claude 主按钮白字（§10.2b）、页面层语义 token --accent-emphasis（§10.2b v4.1）；§7.2 追认 pagination scope；§7.8 highlighted 双源修订；Step 9 任务清单扩容至 10 条，已就绪）
...
`skeleton`、`empty`、`nav`、`shell`、`side-list`、`pro`、`pagination`（Step 3-8 验收追认：
Pagination 当前页态为真实组件场景，`--pagination-current-*` 转正）
...
| highlighted | `focus:` **+ `data-[highlighted]:`（双源并写，染同一 token）** | SelectItem、DropdownMenuItem。v4.1 修订（Step 3-8 验收 `72df764` 追认）：实测当前 radix-ui 指针高亮中 item focus 与 `data-highlighted` 是两条链路（focus 先行、属性下一帧上树），仅写 `focus:` 会漏指针高亮首帧；双源染同一 token 值，无级联冲突。原"统一落到 item focus"前提对当前版本不成立 |
...
### 10.2b 值表 v4 增量修正（第七轮深挖调研 + 四项拍板，2026-07-04）

> 依据：`docs/design/research/` 原始数据存档（飞书 UD 13273 变量全量、Anthropic 官方 MCP 规范全表、
> shadcn new-york-v4 源码合同 + zinc registry）。四项拍板（§16.7）：飞书蓝保留 `#3370FF`、
> shadcn 控件高改 36px、alpha 交互态体系上、clay 用产品级 `#D97757`。
> 本表是对 v3 值表的**增量覆盖**，冲突处以本表为准；由 Step 9 精修切片统一落地。

**机制级：alpha 交互态 token（新增，三 flavor 通用）**

三家产品级交互底/分割线均为 alpha 混合（飞书 N900@8/12%、Claude #1F1E1D@15-40%、shadcn dark 白@10/15%），
新增通用 token，light 用墨 alpha、dark 用白 alpha，flavor 可覆盖强度：

```css
:root {
  --fill-hover: rgba(31, 35, 41, 0.08);      /* 飞书 UD fill-hover 官方值；claude/shadcn 覆盖见下 */
...
消费点迁移（`hover:bg-surface-2` → `hover:bg-(--fill-hover)`）随 Step 9 逐族替换；
`--surface-2` 保留其"静态次级表面"语义（表头底、禁用底），不再兼职交互态。

**feishu 修正行**

| 项 | v3 值 | v4 值（依据） |
| --- | --- | --- |
| 主蓝 | `#3370FF` | **保留 `#3370FF`**（拍板①：admin 实测 + 原型权威；主应用 `#1456F0` 记录在案不采用） |
| --pri-hover 方向 | color-mix 85% black（变深） | **`color-mix(in srgb, var(--pri) 85%, white)`（变浅）**——UD 官方 hover 升阶（#1456F0→#336DF4），pressed 保持变深 |
| hover/pressed 交互底 | surface-2 实色 | `--fill-hover/pressed`（N900@8/12%，UD 官方值） |
| 边框 | 单一 --border `#e5e6eb` | 两级：card `#DEE0E3`(N300) / component `#D0D3D6`(N350)；divider = N900@15%（映射到 --border / --control-border / 新 --divider） |
| selected 底 | --pri-soft 实色 | `--fill-selected`（蓝@10%，UD fill-selected 同构） |

**claude 修正行**

| 项 | v3 值 | v4 值（依据） |
| --- | --- | --- |
| clay 主色 | `#cc785c`（营销） | **`#D97757`**（拍板④：claude.ai CDS 官方 `--cds-clay`） |
| clay active | `#a9583e` | **`#C6613F`**（CDS `--cds-clay-emphasized`） |
| clay soft | `#f8ede7` 实色 | `rgba(217, 119, 87, 0.12)`（alpha 化，与 hexToSoft 派生一致） |
| 边框 | hairline `#E6DFD8` 实色 | **墨色 alpha 三级**：`#1F1E1D` @ 40%（primary）/ 30%（secondary）/ 15%（tertiary，即 0.5px 描边值）——官方 MCP 规范；dark 反转为 `#DEDCD1` 同档 alpha |
| bg 阶梯 | `#FAF9F5`/`#F8F8F6` 两值 | 对齐官方：light bg-primary `#FFFFFF`（工作面）/ secondary `#F5F4ED` / tertiary `#FAF9F5`（画布）；dark `#30302E`/`#262624`/`#141413` |
| focus | coral 15% 晕染 | **保留 coral 晕染**（第六轮拍板不变；官方产品 ring `#141413@70%` 记录在案供 a11y 参考） |
| 主按钮文字色 | 算法深字 `#18181b`（5.2:1） | **白字 `#FFFFFF` 跟随官方**（2026-07-05 拍板：身份复刻优先，与飞书蓝白字同一处理逻辑；3.12:1 on `#D97757` 已入 design-md-lint 白名单，正文禁用该组合作补偿） |

**shadcn 修正行**

| 项 | v3 值 | v4 值（依据） |
| --- | --- | --- |
| 控件高 | 32px | **36px（h-9）**（拍板②：new-york-v4/radix 线源码；32px 系 Base UI 线误采） |
| dark 边框 | 实色 `#27272a` | **白@10%** `oklch(1 0 0 / 10%)`（zinc registry 官方） |
| dark 输入底 | transparent | `--input`(白@15%) 的 30% 叠加（源码 `dark:bg-input/30`） |
| invalid ring | danger-bg | `destructive/20`（dark `/40`）（源码合同） |
| primary hover | color-mix black | `color-mix(in srgb, var(--pri) 90%, transparent)`（源码 `/90`） |

**hover 派生方向裁决**：`--pri-hover`/`--pri-active` 从 :root 单一派生改为 **flavor 可覆盖**——
feishu hover 向白 15%、claude hover 微深 8%（官方"press 才变深，hover 克制"）+ active `#C6613F` 显式、
shadcn hover 90% 透明混合。ACCENTS 的 `active` 字段模式（Step 2R 已建）推广承载显式值。

**页面层语义 token（v4.1 新增，Step 3-8 验收拍板）**：Step 7 曾为通过 pri-ban guard 把页面主色
用途（头像底、目录徽章、权限 chip、refreshing 标签等）挂到 `--nav-item-*`/`--table-action-*` 等
语义不符的 scope（验收 P1-1）。裁决：新增页面层语义组 `--accent-emphasis`（前景强调，默认 = `--pri`）
与 `--accent-emphasis-soft`（软底，默认 = `--pri-soft`，Step 9 起 = `--fill-selected`），
由 Step 9 替换全部洗白点位；`roles/model.ts` 中 add/create 徽章误改 `info` 色的无依据变更一并回归
`--accent-emphasis` 语义。页面层此后只允许消费该组表达业务主色强调，不得再借用组件 scope。

**Step 9 值对照回写（2026-07-05 实施记录）**

| 范围 | Step 3-8 实施值 | research 存档对照 | Step 9 裁决 |
| --- | --- | --- | --- |
| alpha 交互态 | 多处 `surface-2`/`pri-soft` 兼职 hover/selected | `feishu-ud-tokens.json` 的 `--fill-hover/pressed/selected`；Claude MCP 边框/交互均为墨色 alpha；`shadcn-zinc-theme.json` dark 边框/输入为白 alpha | 新增 `--fill-hover/pressed/selected`；交互态迁移到 fill，`surface-2` 回归静态次级面 |
| feishu | hover 走深色派生；border 单档 `#e5e6eb` | `feishu-ud-tokens.json`：primary hover 升阶变浅、line border card/component 两档、divider=N900@15% | `--pri-hover` 向白 15%；`--border/#control-border/--divider` 三值落地 |
| claude | clay `#CC785C` / active `#A9583E` / soft 实色；bg dark 为旧暖黑阶梯 | `claude-ai-cds-tokens.json`：`--cds-clay #D97757`、`--cds-clay-emphasized #C6613F`；`claude-mcp-design-tokens.md`：bg 与 border alpha 全表 | clay/active/soft、bg 阶梯、border alpha 三级、主按钮白字全部按 v4.1 落地 |
| shadcn | radix 线误保留 32px；dark border 实色；invalid ring 旧 danger-bg | `shadcn-source-contracts.md`：h-9=36px、`dark:bg-input/30`、`destructive/20`/dark `/40`；`shadcn-zinc-theme.json`：dark border white@10% | 控件高回 36px；dark border/input/invalid ring 与 primary hover `/90` 按官方源码落地 |
...
### 16.7 三体系深挖调研与四项拍板（第七轮，2026-07-04，v4 依据）

背景：用户要求对飞书与 shadcn 做与 Claude 同深度的规范级调研（Ant Design 级细致度）。成果：

- **飞书**：admin 不用 CSS 变量（编译期字面量），但飞书主应用网页版有 **13273 个变量的完整 UD token 体系**
  （全量存档 `research/feishu-ud-tokens.json`）：N/B 全色阶 + alpha 变体、primary/danger 状态四件套
  （**hover 变浅、pressed 变深**——修正本项目派生方向）、fill 交互态（hover=N900@8%）、边框两级、
  `bg-tips #1F2329` 与原型 tooltip 值一致。主应用蓝 `#1456F0` 与 admin 蓝 `#3370FF` 两代并存。
- **shadcn**：new-york-v4（radix 线）源码合同 + zinc registry 官方值（存档 `research/shadcn-*`）。
  修正第五轮误采：官网 demo 32px 为 Base UI 线，radix 线为 h-9=36px；dark 边框白@10%、输入底 input/30。
- **Claude 补充**（第七轮前置）：Anthropic 官方 MCP Apps Design Token 规范全表 + claude.ai CDS 变量
  （clay 官方值 `#D97757`/`#C6613F`；产品 UI 为系统字体栈，衬线仅营销层——佐证"衬线仅标题层"拍板）。
- **跨体系发现**：三家产品级交互态/边框均为 **alpha 混合体系**（非实色），dark mode 自动成立——
  升级为本项目机制（§10.2b fill token）。
- **四项拍板**（用户，2026-07-04）：① 飞书蓝保留 `#3370FF`；② shadcn 控件高改 36px；
  ③ alpha 交互态体系采纳；④ clay 用产品级 `#D97757`。
- 原始数据无损存档于 `docs/design/research/`（README 即索引）；增量修正见 §10.2b；由 Step 9 落地。
...
Step 2R  Field 值表重定值 + 形态轴机制（§4.8）落地          ✅ 已完成（commit eeea5c9 + cbbb31b，已验收）
         任务清单：
...
Step 3-8 Button / Overlay / Option-Menu / Tabs-Choice / Table-Pro-Shell / 固化
         ✅ 已验收（2026-07-05，commit 42bf450→72df764，有条件通过）：
         四条命令全绿；guard 棘轮 177→30 单调收紧且进 CI；§8 Radix 硬约束 12 条全过；
         三 flavor 机制级差异浏览器实证（feishu 灰底反转无 ring / claude coral 15% 晕染 /
         shadcn 透明底中灰晕染）；Dialog 嵌套 Select、Escape 链、--radix-* 变量、
         shadcn dark 主按钮、自定义亮色 accent 全过；四个值不同压制对 CSSOM 顺序符合 §7.5。
         遗留缺陷全部转入 Step 9：P0-1 三族状态机缺失（渲染恰好正确但依赖变体顺序）、
         P1 洗白/pagination scope（已追认 §7.2）/data-[highlighted]（已回写 §7.8）/sheet 阴影、
         P2 guard 目录遗漏与硬编码清单、剩余 30 处判例标注。
Step 9   精修切片：值表 v4（§10.2b）统一落地 + 验收缺陷修复                   ← 已就绪，待 Codex 开工
         任务清单：
         1. alpha 交互态 token（--fill-hover/pressed/selected）落地 + 逐族消费点迁移
         2. feishu：--pri-hover 方向翻转（向白 15%）；边框两级 + divider alpha；selected 底 alpha 化
         3. claude：clay #cc785c → #D97757 / active #C6613F / soft alpha 化；边框改墨色 alpha 三级；
            bg 阶梯对齐官方（light 画布 #FAF9F5 / 工作面 #FFFFFF / dark #262624 系）；
            主按钮文字色改白字跟随官方（§10.2b 拍板行）
         4. shadcn：控件高 32 → 36px；dark 边框白@10%；dark 输入底 input/30；invalid ring destructive/20
         5. hover 派生方向 flavor 化（§10.2b 裁决）
         6. Step 3-8 各族在实施中所用值与 research/ 存档差异逐项对照回写
         7. 【P0-1】Table 行 / Tabs-line / Choice 三族补 §7.7 状态机钩子（.ui-table-row /
            .ui-tabs-line-trigger / .ui-choice，与 Field 同构：单消费点 + 中间 token 按 §7.5
            声明顺序重赋值），消费点迁移后由浏览器 computed-style 断言收口
         8. 【P1-1】落地页面层语义 token --accent-emphasis / --accent-emphasis-soft（§10.2b v4.1），
            替换全部洗白点位（users/roles model.ts 头像底、MenuTreeTable 目录徽章、
            SubsystemSwitcher builtin 徽章、RoleDetailsPanel refreshing 标签、
            RolePermissionEditor chip/图标/全选）；roles/model.ts add/create 徽章回归主色语义
         9. 【P1-4】Sheet 右抽屉阴影回归方向性 --shadow-drawer（或浏览器视觉定稿后留用 modal 阴影
            并回写 §9.6）；清理孤儿 token
        10. 【P2】guard violationRoots 补 src/app 全域；tokenizedStateFiles 改词边界匹配；
            剩余 baseline 余量（badge/progress/dashboard/login）补判例标注或清零；
            dropdown SubContent 缩进修复
         验收：同 Step 2R 模式（四条命令全绿 + 浏览器矩阵 + 三 flavor 并排截图 + 不退化抽验），
         加两条：①三族状态机压制对 computed-style 断言 ②洗白点位替换后 pri-ban guard 仍绿
```

每个后续切片的纪律追加一条：**动手前先按 §16.5 方法补采该族三站点实测值**，值表没有实测锚定的单元格不许写代码。
Step 9 后进入整体收尾复盘（DoD §14 终验 + 工程复盘）。
