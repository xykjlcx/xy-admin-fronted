...
version: alpha
name: Shadcn-Admin
description: shadcn/ui 官方（new-york / zinc）设计体系的后台适配版。纯白画布 + zinc 中性灰阶，主色即墨黑。输入框透明底 + 浅灰边，聚焦是中灰边 + 3px 半透明晕染 ring（不是主色边框）。10px 圆角、32px 紧凑控件、无阴影文化。整体气质：工程师的克制精确，零装饰。
colors:
  primary: "#18181b"
  primary-fg: "#fafafa"
  canvas: "#ffffff"
...
两条组件线均采），详见 spec §16.5。dark mode：官方 zinc dark（画布 #09090B、表面 #18181B、
primary 反转为 #FAFAFA 配墨字）——本项目 `tokens.css` shadcn dark block 与 accent `priDark` 已实现。
...
- 控件 32px 紧凑高度、10px 圆角（比常见 6/8px 大而圆润）、无阴影。
...
4px 网格，控件高 32px、水平内边距 10px——三个体系里最紧凑的控件尺寸，但留白比飞书多，
密度感来自控件小巧而非行距压缩。
...
- **`text-input`**：**透明底** + 1px `{colors.hairline}` 边、10px 圆角、32px 高。
  聚焦 `text-input-focused`：边框转中灰（≈zinc-500，不是黑）+ **3px `{colors.ring}` 50% 透明晕染 ring**。
- **`button-primary`**：墨黑底 `{colors.primary}` + `{colors.on-primary}` 字、32px 高、10px 圆角、无阴影。
