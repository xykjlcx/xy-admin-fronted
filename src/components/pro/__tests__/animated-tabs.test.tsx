...
test('AnimatedTabs 渲染可访问 tab 并提供动画指示条', () => {
  ...
  expect(screen.getByRole('tab', { name: '角色与权限' })).not.toHaveClass('text-(--tabs-line-trigger-fg-active)');
  expect(screen.getByRole('tab', { name: '管理员权限' })).toHaveClass('ui-tabs-line-trigger');
  expect(screen.getByRole('tab', { name: '管理员权限' })).not.toHaveClass('text-(--tabs-line-trigger-fg)');
  expect(screen.getByRole('tab', { name: '管理员权限' })).not.toHaveClass('hover:text-(--tabs-line-trigger-fg-hover)');
