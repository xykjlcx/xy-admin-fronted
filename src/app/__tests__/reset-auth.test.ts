...
// 防回归：换账号不串权限。若统一会话服务不清 me 缓存，ensureQueryData 会返回上个账号的缓存。
test('预置 admin me 缓存 → viewer 登录统一会话服务 → beforeLoad 取到 viewer 权限', async () => {
  ...
  await sessionCredentialService.replace(token); // 复用登录成功路径的会话切换逻辑
...
test('统一会话服务清空所有业务缓存并写入新 token', async () => {
  ...
  await sessionCredentialService.replace('new-token');
