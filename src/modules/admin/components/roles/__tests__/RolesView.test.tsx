...
import { RolesView, type RolesViewProps } from '@/modules/admin/components/roles/RolesView';
...
test('角色成员和操作日志 tab 展示对应数据', async () => {
  ...
  await userEvent.click(screen.getByRole('button', { name: '角色成员 · 1' }));
...
test('管理员权限 tab 可以新增管理员角色', async () => {
  ...
  await userEvent.click(screen.getByRole('button', { name: '管理员权限' }));
