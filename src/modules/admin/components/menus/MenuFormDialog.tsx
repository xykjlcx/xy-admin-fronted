...
import {
  nextSiblingSort,
} from '@/modules/admin/components/menus/menu-management-model';
import type { ManagedMenuType, UpdateMenuInput } from '@/modules/admin/api/menu.api';
import type { MenuRecord } from '@/modules/types';

const routeOptions = [
  { value: '/admin/dashboard', label: '企业概览' },
  { value: '/admin/users', label: '成员与部门' },
  { value: '/admin/roles', label: '角色与权限' },
  { value: '/admin/menus', label: '菜单管理' },
] satisfies { value: NonNullable<MenuRecord['path']>; label: string }[];
...
function isDescendant(menus: MenuRecord[], candidateId: string, ancestorId: string): boolean {
  const candidate = menus.find((menu) => menu.id === candidateId);
  if (!candidate?.parentId) return false;
  if (candidate.parentId === ancestorId) return true;
  return isDescendant(menus, candidate.parentId, ancestorId);
}

function parentOptions(type: ManagedMenuType, menus: MenuRecord[], locale: string, editingId?: string) {
  const rootOption = { id: '', label: '根级菜单' };
  if (type === 'dir') return [rootOption];

  const candidates = menus
    .filter((menu) => {
      if (menu.id === editingId) return false;
      if (editingId && isDescendant(menus, menu.id, editingId)) return false;
...
export function MenuFormDialog({
  ...
  const currentParentOptions = useMemo(
    () => parentOptions(draft.type, menus, locale, editingId),
    [draft.type, editingId, locale, menus],
  );

  const patchDraft = (patch: Partial<DraftState>) => setDraft((current) => ({ ...current, ...patch }));
  const changeType = (type: ManagedMenuType) => {
    const options = parentOptions(type, menus, locale, editingId);
