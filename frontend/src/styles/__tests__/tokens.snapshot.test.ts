...
// sera 第四风格（S5 北极星）：颜色抄 shadcn zinc 基线，差异化全交几何/排印 + 输入族底边线。
test('sera 第四风格 profile 落地：直角/大写/下划线关键值', () => {
  ...
  expect(css).toContain('--label-tracking: 0.025em;');
  expect(css).toContain('--table-header-transform: uppercase;');
  expect(css).toContain('--table-header-tracking: 0.05em;');
  expect(css).toContain('--title-transform: uppercase;');
  expect(css).toContain('--title-tracking: 0.05em;');
  // 输入族（底边线）：透明底 + hairline 底边；invalid 无红 ring 只留底边红（对抗审 V2）
  const seraBlock = css.slice(css.indexOf("[data-flavor='sera'] {"));
  expect(seraBlock).toContain('--field-bg: transparent;');
  expect(seraBlock).toContain('--field-border: var(--border);');
  expect(seraBlock).toContain('--field-ring-invalid: transparent;');
...
test('主题状态页暴露 Dialog title 可截图矩阵（S5 批1 --title-* 载体，防误删）', () => {
  expect(themeStatesSource).toContain('data-testid="dialogTitleMatrix"');
  expect(themeStatesSource).toContain('data-slot="dialog-title"');
  // 截图载体不再硬编码排印类串，改 import dialog.tsx 的 dialogTitleClassName 单一真相源（防漂移）
  expect(themeStatesSource).toContain('dialogTitleClassName');
  expect(dialogSource).toContain('const dialogTitleClassName');
});
...
test('排印 transform/tracking 挂点：6 处消费方按 S5 批1 合同接线（默认 none/normal 零视觉变化）', () => {
  // button / label / 表头(ui+shell) / dialog title / card title 均以任意属性类消费对应 token 束
  expect(buttonSource).toContain('[text-transform:var(--button-transform)]');
  expect(buttonSource).toContain('[letter-spacing:var(--button-tracking)]');
  expect(labelSource).toContain('[text-transform:var(--label-transform)]');
  expect(labelSource).toContain('[letter-spacing:var(--label-tracking)]');
  expect(tableSource).toContain('[text-transform:var(--table-header-transform)]');
  expect(tableSource).toContain('[letter-spacing:var(--table-header-tracking)]');
  expect(tableShellSource).toContain('[text-transform:var(--table-header-transform)]');
  expect(tableShellSource).toContain('[letter-spacing:var(--table-header-tracking)]');
  expect(dialogSource).toContain('[text-transform:var(--title-transform)]');
  expect(dialogSource).toContain('[letter-spacing:var(--title-tracking)]');
  expect(cardSource).toContain('[text-transform:var(--title-transform)]');
  expect(cardSource).toContain('[letter-spacing:var(--title-tracking)]');
  // label peer 豁免（收窄版 D3）：只撤 transform+tracking 两条，绝不含 text-sm/font-normal（对抗审 V5-1 回归点）
  expect(labelSource).toContain('peer-data-[slot=checkbox]:[text-transform:none]');
  expect(labelSource).toContain('peer-data-[slot=checkbox]:[letter-spacing:normal]');
  expect(labelSource).toContain('peer-data-[slot=radio-group-item]:[text-transform:none]');
  expect(labelSource).toContain('peer-data-[slot=switch]:[text-transform:none]');
  expect(labelSource).not.toContain('peer-data-[slot=checkbox]:text-sm');
  expect(labelSource).not.toContain('peer-data-[slot=checkbox]:font-normal');
});
...
test('Table / Pro / Shell 族 token 与 Step 7 合同落地', () => {
  const step7Tokens = [
    ...
    '--pro-timeline-marker-bg: var(--surface-2);',
    '--pro-timeline-marker-border: var(--border);',
    '--pro-timeline-marker-active: var(--pri);',
  ...
  for (const source of [membersSceneSource, deptSceneSource]) {
    ...
    expect(source).not.toContain('px-6 py-[calc(18px*var(--app-scale))]');
  }
  ...
  expect(globalCss).toContain('--page-side-py: calc(12px * var(--app-scale));');
