...
import { ok, noContent, biz } from '@/mocks/http';
...
export const sessionHandlers = [
  http.post('/api/auth/login', async ({ request }) => {
    ...
    if (!parsed.success) return biz({ status: 400, code: 'auth.login.invalid', detail: '登录参数不完整' });
  ...
  http.post('/api/auth/sms-code', async ({ request }) => {
    ...
    if (!parsed.success) return biz({ status: 400, code: 'auth.phone.invalid', detail: '请输入正确的手机号' });
