...
test('GET /api/users 支持部门、状态、关键词过滤和分页', async () => {
  ...
  expect(res.list.every((u) => u.deptId === 'rd')).toBe(true);
  expect(res.list.every((u) => u.status === 'active')).toBe(true);
  expect(res.list.every((u) => u.name.includes('李'))).toBe(true);
});
...
test('GET /api/users 支持仅查询当前部门直属成员', async () => {
  ...
  expect(recursive.total).toBeGreaterThan(directOnly.total);
  expect(directOnly.list.every((u) => u.deptId === 'rd')).toBe(true);
});
...
test('POST /api/users 写入后能被列表查询读回', async () => {
  ...
  expect(created.name).toBe('测试成员');

  const list = (await (await fetch('/api/users?keyword=测试成员')).json()) as Page<UserRow>;
  expect(list.list.some((u) => u.id === created.id)).toBe(true);
});
